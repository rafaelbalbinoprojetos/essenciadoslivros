...
const panelClass = "rounded-[28px] border border-white/10 bg-[linear-gradient(145deg,rgba(29,25,19,0.96),rgba(12,12,10,0.94))] shadow-[0_24px_80px_-45px_rgba(0,0,0,0.95)]";
    ...
    <article className="-mx-4 -mt-6 overflow-hidden bg-[#090907] text-[#f5ead7] md:-mx-8">
      <section className="relative flex min-h-[760px] items-end overflow-hidden px-5 pb-8 pt-28 sm:px-10 lg:min-h-[calc(100vh-2rem)] lg:px-16 lg:pb-12">
        ...
        <div className="absolute right-5 top-6 z-20 flex gap-2 sm:right-10 lg:right-16">
          <button type="button" onClick={handleShare} aria-label="Compartilhar" className="flex h-12 w-12 items-center justify-center rounded-2xl border border-white/20 bg-black/35 backdrop-blur-md transition hover:border-[rgb(var(--color-accent-primary))]"><Share2 className="h-5 w-5" /></button>
          {admin && (
            <div className="relative">
              <button type="button" onClick={() => setAdminMenuOpen((open) => !open)} aria-label="Opções administrativas" className="flex h-12 w-12 items-center justify-center rounded-2xl border border-white/20 bg-black/35 backdrop-blur-md transition hover:border-[rgb(var(--color-accent-primary))]"><MoreVertical className="h-5 w-5" /></button>
              {adminMenuOpen && (
                <div className="absolute right-0 top-14 w-48 overflow-hidden rounded-2xl border border-white/15 bg-[#15130f]/95 p-2 shadow-2xl backdrop-blur-xl">
                  <Link to={`/biblioteca/${bookId}/editar`} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm hover:bg-white/10"><Pencil className="h-4 w-4" />Editar obra</Link>
        ...
        <div className="relative z-10 w-full max-w-7xl">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-5 font-display text-5xl font-semibold leading-[0.95] text-[#fff8eb] sm:text-6xl lg:text-8xl">{book.titulo}</h1>
            <p className="mt-5 text-lg font-medium text-[rgb(var(--color-accent-primary))]">✧ {book.genero?.nome || "Acervo Essência"}</p>
            <p className="mt-3 max-w-xl text-base leading-7 text-[#e5d9c6]/90 sm:text-lg">{book.subtitulo || book.sinopse || `Uma exposição dedicada à essência de ${book.titulo}.`}</p>
            <p className="mt-5 text-sm uppercase tracking-[0.2em] text-[#d2c3ad]">{book.autor?.nome || "Autoria não informada"}</p>

            <div className="mt-7 flex flex-wrap items-center gap-x-6 gap-y-4 text-sm text-[#e5d9c6]">
            ...
            <div className="mt-6 flex flex-wrap items-center gap-6">
              <div>
                ...
                <p className="text-xs text-[#b9aa93]">{reviews.count.toLocaleString("pt-BR")} avaliações</p>
              </div>
      ...
      <div className="mx-auto -mt-3 max-w-7xl space-y-7 px-4 pb-16 sm:px-8 lg:px-12">
        <nav className={`${panelClass} grid grid-cols-2 overflow-hidden p-2 sm:grid-cols-5`} aria-label="Experiências disponíveis">
          {experiences.map(({ label, caption, icon, available, action }) => (
            <button key={label} type="button" onClick={action} disabled={!available} className="group flex min-h-28 flex-col items-center justify-center rounded-2xl px-3 py-4 text-center transition hover:bg-white/5 disabled:cursor-not-allowed disabled:opacity-30">
              ...
              <span className="text-xs text-[#b9aa93]">{caption}</span>
            </button>
          ))}
        </nav>
        ...
        <section id="curadoria" className={`${panelClass} scroll-mt-24 overflow-hidden p-7 sm:p-10`}>
          <div className="grid items-center gap-8 lg:grid-cols-[1fr_280px]">
            <div className="max-w-4xl">
              ...
              <blockquote className="mt-5 font-display text-2xl italic leading-relaxed text-[#eadbc4] sm:text-3xl">“{book.curadoria_editorial || book.sinopse || `Uma obra escolhida para ser vivida, lembrada e redescoberta.`}”</blockquote>
              <p className="mt-5 text-sm text-[rgb(var(--color-accent-primary))]">— Essência dos Livros</p>
            </div>
            <img src={coverSrc} alt={`Capa de ${book.titulo}`} className="mx-auto aspect-[3/4] w-full max-w-[230px] rotate-2 rounded-xl border border-white/15 object-cover shadow-2xl" />
          </div>
        </section>

        <section className={`${panelClass} p-6 sm:p-8`}>
          <p className={eyebrowClass}>Informações essenciais</p>
          <div className="mt-6 grid divide-y divide-white/10 sm:grid-cols-5 sm:divide-x sm:divide-y-0">
            {infoItems.map(({ label, value, icon }) => (
              <div key={label} className="px-4 py-5 text-center first:pl-0 last:pr-0 sm:py-2">
                ...
                <p className="mt-3 text-[9px] font-bold uppercase tracking-[0.2em] text-[#9f927e]">{label}</p>
                <p className="mt-1 text-sm capitalize text-[#eadfcf]">{value}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {(book.voce_sabia || book.legado) && (
          <div className="grid gap-5 lg:grid-cols-2">
            {book.voce_sabia && <section className={`${panelClass} p-7`}><p className={`${eyebrowClass} flex items-center gap-2`}><Lightbulb className="h-4 w-4" />Você sabia?</p><p className="mt-5 font-display text-xl leading-relaxed text-[#e5d9c6]">{book.voce_sabia}</p></section>}
            {book.legado && <section className={`${panelClass} p-7`}><p className={`${eyebrowClass} flex items-center gap-2`}><Trophy className="h-4 w-4" />Legado</p><p className="mt-5 font-display text-xl leading-relaxed text-[#e5d9c6]">{book.legado}</p></section>}
          </div>
        )}

        {timeline.length > 0 && (
          <section className={`${panelClass} overflow-hidden p-6 sm:p-8`}>
            <div className="flex items-center justify-between"><p className={eyebrowClass}>Linha do tempo</p><span className="text-xs text-[#8f826f]">{timeline.length} marcos</span></div>
            <div className="mt-8 grid gap-0 overflow-x-auto pb-3" style={{ gridTemplateColumns: `repeat(${timeline.length}, minmax(170px, 1fr))` }}>
              {timeline.map((item, index) => (
                <article key={`${item.ano}-${item.titulo}-${index}`} className="relative border-t border-[rgb(var(--color-accent-primary))]/50 px-5 pt-7 first:pl-0 last:pr-0">
                  ...
                  {item.descricao && <p className="mt-1 text-xs leading-5 text-[#a99d8c]">{item.descricao}</p>}
                </article>
              ))}
            </div>
          </section>
          ...
          <section className={`${panelClass} p-6 sm:p-8`}>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {gallery.map((item, index) => (
                <figure key={`${item.titulo}-${index}`} className="group overflow-hidden rounded-2xl border border-white/10 bg-black/30">
                  ...
                  <figcaption className="p-3"><p className="text-sm font-medium">{item.titulo || `Objeto ${index + 1}`}</p>{item.descricao && <p className="mt-1 text-xs text-[#9f927e]">{item.descricao}</p>}</figcaption>
                </figure>
              ))}
            </div>
          </section>
        ...
        {hasCinematic && (
          <section id="cinematica" className={`${panelClass} scroll-mt-24 overflow-hidden`}>
            <header className="flex flex-wrap items-center justify-between gap-5 border-b border-white/10 p-6 sm:p-8">
              <div><p className={eyebrowClass}>Memória cinematográfica</p><h2 className="mt-2 font-display text-3xl">{book.titulo_cinematico || narrative?.titulo || book.titulo}</h2><p className="mt-2 max-w-2xl text-sm text-[#a99d8c]">{book.descricao_cinematica || narrative?.descricao || "Uma narrativa construída em cenas."}</p></div>
            ...
            <ol className="grid gap-px bg-white/10 lg:grid-cols-2">
              {narrativeTracks.map((track, index) => {
                const active = currentTrack?.id === `narrative-${track.id}`;
                return (
                  <li key={track.id} className="bg-[#11100d]">
                    <button type="button" onClick={() => handleNarrative(index)} className={`flex w-full items-center gap-4 p-5 text-left transition hover:bg-white/5 ${active ? "bg-white/5" : ""}`}>
                      <span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full border ${active ? "border-[rgb(var(--color-accent-primary))] bg-[rgb(var(--color-accent-primary))] text-white" : "border-white/20 text-[rgb(var(--color-accent-primary))]"}`}>{active && playerIsPlaying ? <Pause className="h-4 w-4 fill-current" /> : <Play className="h-4 w-4 fill-current" />}</span>
                      <span className="min-w-0 flex-1"><span className="text-[9px] font-bold uppercase tracking-[0.24em] text-[#867967]">Cena {index + 1}</span><span className="mt-1 block truncate text-sm font-semibold">{track.titulo}</span>{track.descricao && <span className="mt-1 block line-clamp-1 text-xs text-[#9f927e]">{track.descricao}</span>}</span>
                      {formatTrackDuration(track.duracao_segundos) && <span className="text-xs text-[#867967]">{formatTrackDuration(track.duracao_segundos)}</span>}
                    </button>
                  </li>
                );
              })}
            ...
            <div className="mt-6 divide-y divide-white/10">
              {soundtrack.map((track, index) => (
                <div key={`${track.titulo}-${index}`} className="flex items-center gap-4 py-4">
                  <span className="flex h-10 w-10 items-center justify-center rounded-full border border-white/15 text-[rgb(var(--color-accent-primary))]">{String(index + 1).padStart(2, "0")}</span>
                  <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{track.titulo || `Faixa ${index + 1}`}</p><p className="text-xs text-[#928571]">{track.artista || track.descricao || "Essência dos Livros"}</p></div>
                  {track.audio_url && <a href={track.audio_url} target="_blank" rel="noreferrer" className="rounded-full border border-white/15 px-4 py-2 text-xs hover:border-[rgb(var(--color-accent-primary))]">Ouvir</a>}
                </div>
              ))}
            </div>
          </section>
        ...
        {related.length > 0 && (
          <section className={`${panelClass} p-6 sm:p-8`}>
            ...
            <div className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
              {related.map((item) => (
                <Link key={item.id} to={`/biblioteca/${item.id}`} className="group overflow-hidden rounded-2xl border border-white/10 bg-black/30 transition hover:-translate-y-1 hover:border-[rgb(var(--color-accent-primary))]/60">
                  <img src={ensureCoverSrc(item.capa_url || item.capa_cinematica_url)} alt={item.titulo} className="aspect-[3/4] w-full object-cover" />
                  <div className="p-4"><h3 className="line-clamp-2 font-display text-lg">{item.titulo}</h3><p className="mt-1 truncate text-xs text-[#9f927e]">{item.autor?.nome || "Essência dos Livros"}</p></div>
                </Link>
              ))}
            </div>
          </section>
      ...
      {confirmandoExclusao && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl border border-red-500/30 bg-[#17130f] p-6 shadow-2xl">
            <h2 className="font-display text-2xl">Excluir esta obra?</h2>
            <p className="mt-3 text-sm leading-6 text-[#b9aa93]">“{book.titulo}” e seus arquivos associados serão removidos permanentemente. Esta ação não pode ser desfeita.</p>
            <div className="mt-6 flex justify-end gap-3">
              <button type="button" onClick={() => setConfirmandoExclusao(false)} disabled={excluindo} className="rounded-xl border border-white/15 px-4 py-2 text-sm">Cancelar</button>
